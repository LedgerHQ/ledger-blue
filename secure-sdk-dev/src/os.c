 ...
 *
 *  Filler only allowed at the end of the last hid report of a apdu chain in
 * each direction.
 *
 ...
 *
 *  Tags:
 *  Direction:Host>Token T:0x00 V:no  Get protocol version big endian encoded.
 * Replied with a protocol-version. Channel identifier is ignored for this
 * command.
 *  Direction:Token>Host T:0x00 V:yes protocol-version-4-bytes-big-endian.
 * Channel identifier is ignored for this reply.
 *  Direction:Host>Token T:0x01 V:no  Allocate channel. Replied with a channel
 * identifier. Channel identifier is ignored for this command.
 *  Direction:Token>Host T:0x01 V:yes channel-identifier-2-bytes. Channel
 * identifier is ignored for this reply.
 *  Direction:*          T:0x02 V:no  Ping. replied with a ping. Channel
 * identifier is ignored for this command.
 *  NOTSUPPORTED Direction:*          T:0x03 V:no  Abort. replied with an abort
 * if accepted, else not replied.
 *  Direction:*          T:0x05
 * V=<sequence-idx-U16><seq==0?totallength:NONE><apducontent> APDU
...
io_usb_hid_receive(io_send_t sndfct, unsigned char *buffer, unsigned short l) {
    ...
    case 0x05:
        ...
        if (G_io_hid_chunk[3] != (G_io_usb_hid_sequence_number >> 8) ||
            G_io_hid_chunk[4] != (G_io_usb_hid_sequence_number & 0xFF)) {
            // ignore packet
            goto apdu_reset;
        }
        // cid, tag, seq
        ...
        if (G_io_usb_hid_sequence_number == 0) {
            ...
            G_io_usb_hid_total_length =
                (G_io_hid_chunk[5] << 8) + (G_io_hid_chunk[6] & 0xFF);
            // check for invalid length encoding (more data in chunk that
            // announced in the total apdu)
            ...
            // copy data
            os_memmove(G_io_usb_hid_current_buffer, G_io_hid_chunk + 7, l);
        } else {
            // check for invalid length encoding (more data in chunk that
            // announced in the total apdu)
            if (l > G_io_usb_hid_remaining_length) {
            ...
            // append content
            os_memmove(G_io_usb_hid_current_buffer, G_io_hid_chunk + 5, l);
        }
        // factorize (f)
...
unsigned short io_usb_hid_exchange(io_send_t sndfct, unsigned short sndlength,
    ...
    while (sndlength) {
        ...
        if (G_io_usb_hid_sequence_number == 0) {
            l = ((sndlength > IO_HID_EP_LENGTH - 7) ? IO_HID_EP_LENGTH - 7
                                                    : sndlength);
            G_io_hid_chunk[5] = sndlength >> 8;
            G_io_hid_chunk[6] = sndlength;
            os_memmove(G_io_hid_chunk + 7, G_io_usb_hid_current_buffer, l);
            G_io_usb_hid_current_buffer += l;
            sndlength -= l;
            l += 7;
        } else {
            l = ((sndlength > IO_HID_EP_LENGTH - 5) ? IO_HID_EP_LENGTH - 5
                                                    : sndlength);
            os_memmove(G_io_hid_chunk + 5, G_io_usb_hid_current_buffer, l);
            G_io_usb_hid_current_buffer += l;
            sndlength -= l;
            l += 5;
        }
...
void os_xor(void *dst, void WIDE *src1, void WIDE *src2,
            unsigned short length) {
